    ...
    if os.path.exists(json_dir_path):
        ...
        for file in files:
            ...
            for prk_data in data['PrkSttusInfo']:
                ...
                if '아파트' in park_nm:
                    print(f"xxxxx {park_nm} 제외 xxxxxx")
                    continue
...
def fun_save(**kwargs):
    ...
    startpage = kwargs['value']
    dir_path = "/opt/airflow/Data/Info/Tdata"
    ...
    try:
        ...
        files = files_sort[startpage-1:startpage+500-1]
        ...
        for file in files:
            ...
            try:
                ...
                print(f"{file} 파일이 비어 있거나 잘못된 형식입니다. 작업을 수행하지 않습니다.")
                continue

            for _, row in df.iterrows():
                park_id = row['park_id'] if pd.notna(row['park_id']) else ""
                park_nm = row['park_nm'] if pd.notna(row['park_nm']) else None
                park_addr = row['park_addr'] if pd.notna(row['park_addr']) else ""
                park_la = row['park_la'] if pd.notna(row['park_la']) else 0.0
                park_lo = row['park_lo'] if pd.notna(row['park_lo']) else 0.0
                #page_no = row['page_no'] if pd.notna(row['page_no']) else None

                select_query = f"SELECT park_id FROM parkingarea_info WHERE park_id = %s"
                cursor.execute(select_query, (park_id,))
                result = cursor.fetchone()

                if result:
                    update_query =f"""
                    UPDATE parkingarea_info
                    SET
                        park_nm = %s,
                        park_addr = %s,
                        park_la = %s,
                    ...
                    WHERE park_id = %s
                    """
                    cursor.execute(update_query, (park_nm, park_addr, park_la, park_lo, park_id))
                    #print(f"ID {park_id} 업데이트 성공")

                else:
                    insert_query = f"""
                    INSERT INTO parkingarea_info (park_id, park_nm, park_addr, park_la, park_lo)
                    VALUES (%s, %s, %s, %s, %s)
                    """
                    cursor.execute(insert_query, (park_id, park_nm, park_addr, park_la, park_lo))
